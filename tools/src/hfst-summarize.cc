...
#               endif
	...
	fprintf(message_out, "%-35s%s", "  -o, --output=OUTFILE", "Print properties to OUTFILE\n");
	// fprintf(message_out, (tool-specific options and short descriptions)
	fprintf(message_out, "\n");
	print_common_unary_program_parameter_instructions(message_out);
	fprintf(stderr, "\n");
	print_more_info(message_out, "Summarize");
	fprintf(stderr, "\n");
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) {
				...
				char line[200];
				sprintf(line, "%-32s%s", "weighted", "n\n");
				outstream << line;				
				if (transducer_has_symbol_table)
				  sprintf(line, "%-32s%s", "has HFST symbol table", "y\n");
				else
				  sprintf(line, "%-32s%s", "has HFST symbol table", "n\n");
				outstream << line;				
				HFST::print_properties(input, outstream);
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) 
			{
				...
				if (transducer_has_symbol_table)
				  ...
				  sprintf(line, "%-32s%s", "has HFST symbol table", "n\n");
				outstream << line;
				HWFST::print_properties(input, outstream);
