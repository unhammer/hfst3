...
//!
//! @brief Transducer determinization command line tool
//!
//! @author HFST Team
...
#               endif
	...
	/*fprintf(message_out,
		   ...
		   "\n", program_name);*/
	fprintf(stderr, "\n");
	print_more_info(message_out, "Determinize");
	fprintf(stderr, "\n");
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) {
				...
				VERBOSE_PRINT("Determinizing...\n");
				
				HFST::TransducerHandle result = HFST::determinize(input);
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) 
			{
				...
				HWFST::TransducerHandle result = HWFST::determinize(input);
