...
//!
//! @brief Epsilo removal command line tool
//!
//! @author HFST Team
...
#               endif
	...
	fprintf(stderr, "\n");
	print_more_info(message_out, "RemoveEpsilons");
	fprintf(stderr, "\n");
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) {
				...
				VERBOSE_PRINT("Removing epsilons...\n");
				// add your code here

				HFST::TransducerHandle result = HFST::remove_epsilons(input);
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) 
			{
				...
				HWFST::TransducerHandle result = HWFST::remove_epsilons(input);
