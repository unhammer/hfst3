...
//!
//! @brief Transducer conjunction command line tool
//!
//! @author HFST Team
...
#               endif
		...
		fprintf(stderr, "\n");
		print_more_info(message_out, "Conjunct");
		fprintf(stderr, "\n");
		/*fprintf(message_out,
		   "\n"
		   "Examples:\n"
		   "  %s -o isection.hfst first.hfst second.hfst  intersects "
...
#include "hfst-common-options.h"
		...
		int option_index = 0;
		char c = getopt_long(argc, argv, "1:2:dhno:qvVsDW:",
...
int
conjunct_streams(std::istream& leftstream, std::istream& rightstream, std::ostream& outstream)
{
	...
	if (format_type == SFST_FORMAT)
	{
		...
		try {
			...
			while (true) {
				...
				if (nth_stream < 2)
				{
					VERBOSE_PRINT("Conjuncting...\n");
				}
				else
				{
					VERBOSE_PRINT("Conjuncting... %zu\r", nth_stream);
				}
				HFST::TransducerHandle comp = 
				  HFST::intersect(HFST::copy(left), HFST::copy(right));
		...
		try {
			...
			while (true) 
			{
				...
				HWFST::TransducerHandle comp = 
				  HWFST::intersect(HWFST::copy(left), HWFST::copy(right));
				if (write_symbol_table && !use_numbers)
...
int main( int argc, char **argv ) {
	...
	if ((leftfile != stdin) && (rightfile != stdin))
	{
		...
		if (outfile != stdout)
		{
			...
			retval = conjunct_streams(leftstream, rightstream, outstream);
		}
		else
		{
			retval = conjunct_streams(leftstream, rightstream, std::cout);
		}
		return retval;
	}
	else if (leftfile != stdin)
	{
		...
		if (outfile != stdout)
		{
			...
			fbout.open(outfilename, std::ios::out);
			std::ostream outstream(&fbout);
			retval = conjunct_streams(leftstream, std::cin, outstream);
		}
		else
		{
			retval = conjunct_streams(leftstream, std::cin, std::cout);
		}
		return retval;
	}
	else if (rightfile != stdin)
	{
		...
		if (outfile != stdout)
		{
			...
			fbout.open(outfilename, std::ios::out);
			std::ostream outstream(&fbout);
			retval = conjunct_streams(std::cin, rightstream, outstream);
		}
		else
		{
			retval = conjunct_streams(std::cin, rightstream, std::cout);
		}
		return retval;
	}
	free(leftfilename);
