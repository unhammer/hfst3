...
//!
//! @brief Transducer equivalent reweighting command line tool
//!
//! @author HFST Team
...
print_usage(const char *program_name)
{
	...
	fprintf(message_out, "Usage: %s [OPTIONS...] [INFILE]\n"
		"Push weights of a transducer towards initial or final state(s)\n"
		"\n", program_name);

	print_common_program_options(message_out);
#               if DEBUG
	fprintf(message_out,
		"%-32s%s", "  -d, --debug", "Print debugging messages and results\n"
		);
#               endif
	print_common_unary_program_options(message_out);
	// fprintf(message_out, (tool-specific options and short descriptions)
	fprintf(message_out, "%-35s%s", "  -p, --push=DIRECTION", "Push towards DIRECTION\n");
	fprintf(message_out, "\n");
	print_common_unary_program_parameter_instructions(message_out);
	fprintf(message_out, "DIRECTION is {start, initial} or {end, final}.\n");
	fprintf(stderr, "\n");
	print_more_info(message_out, "PushWeights");
	fprintf(stderr, "\n");
...
#include "hfst-common-unary-options.h"
		...
		int option_index = 0;
		// add tool-specific options here 
		char c = getopt_long(argc, argv, "dhi:o:sqvVR:DW:p:l",
...
#include "hfst-common-unary-cases.h"
		...
		case 'p':
			dir = hfst_strdup(optarg);
			if ( (strncasecmp(dir, "end", 1) == 0) ||
				 (strncasecmp(dir, "final", 1) == 0) )
			{
				push_initial = false;
			}
			else if ( (strncasecmp(dir, "start", 1) == 0) ||
					  (strncasecmp(dir, "initial", 1) == 0) )
			{
				push_initial = true;
			}
			else
			{
				fprintf(message_out, "unknown push direction %s\n"
									 "should be one of start, initial"
									 "end or final\n",
						dir);
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) {
				...
				VERBOSE_PRINT("Ignoring unweighted...\n");
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) 
			{
				...
				VERBOSE_PRINT("Pushing weights towards ");
				if (push_initial) {
				  VERBOSE_PRINT("initial");
				}
				else {
				  VERBOSE_PRINT("final");
				}
				...
				HWFST::TransducerHandle result;
				  result = HWFST::push_weights(input, push_initial);
