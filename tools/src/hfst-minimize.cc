...
//!
//! @brief Transducer minimization command line tool
//!
//! @author HFST Team
...
#               endif
	...
	fprintf(stderr, "\n");
	print_more_info(message_out, "Minimize");
	fprintf(stderr, "\n");
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) {
				...
				HFST::TransducerHandle result = HFST::minimize(input);
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) 
			{
				...
				HWFST::TransducerHandle result = HWFST::minimize(input);
