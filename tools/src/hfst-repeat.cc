//! @file hfst-repeat.cc
//!
//! @brief Repeating command line tool
//!
//! @author HFST Team
...
#               endif
	...
	fprintf(message_out, "%-35s%s", "  -f, --from=NUMBER", "Repeat at least NUMBER times\n");
	fprintf(message_out, "%-35s%s", "  -t, --to=NUMBER",   "Repeat at most NUMBER times\n");
	fprintf(message_out, "\n");
	...
	fprintf(stderr, "\n");
	print_more_info(message_out, "Repeat");
	fprintf(stderr, "\n");
...
#include "hfst-common-unary-options.h"
		...
		int option_index = 0;
		// add tool-specific options here 
		char c = getopt_long(argc, argv, "dhi:o:sqvVR:DW:f:t:",
...
#include "hfst-common-unary-cases.h"
		...
		case 'f':
			from = hfst_strtoweight(optarg);
			break;
		case 't':
			to = hfst_strtoweight(optarg);
			break;
		case '?':
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) {
				...
				VERBOSE_PRINT("Repeating...\n");
				// add your code here

				HFST::TransducerHandle result;

				HFST::TransducerHandle tr_to;
				if (to == INFINITY)
				  tr_to = HFST::repeat_star(HFST::copy(input));
				else
				  tr_to = HFST::repeat_le_n(HFST::copy(input), (int)to);


				if (from == (double)0)
				  result = tr_to;
				else {
				  HFST::TransducerHandle tr_from = HFST::repeat_le_n(input, ((int)from)-1);
				  result = HFST::subtract( tr_to, tr_from );
				}
		...
		try {
		  ...
		  bool transducer_has_symbol_table=false;
			...
			while (true) 
			{
				...
				HWFST::TransducerHandle tr_to;
				if (to == INFINITY)
				  tr_to = HWFST::repeat_star(HWFST::copy(input));
				else
				  tr_to = HWFST::repeat_le_n(HWFST::copy(input), (int)to);

				if (from == (double)0)
				  result = tr_to;
				else {
				  HWFST::TransducerHandle tr_from = HWFST::repeat_le_n(input, ((int)from)-1);
				  //fprintf(stderr, "tr_to and tr_from:\n");
				  //HWFST::print_transducer_number(tr_to, cerr);
				  //fprintf(stderr, "--\n");
				  //HWFST::print_transducer_number(tr_from, cerr);

				  result = HWFST::subtract( tr_to, tr_from );
				}
