...
void Transducer::expand_node( Node *origin, Label &l, Node *target, Transducer *a, StringSymbolSet &s )
{
  if ( l.lower_char() == 1 && l.upper_char() == 1 )     // cross product "?:?"
    {
      for (StringSymbolSet::iterator it1 = s.begin(); it1 != s.end(); it1++) 
	{
	  int inumber = alphabet.symbol2code(it1->c_str());
	  for (StringSymbolSet::iterator it2 = s.begin(); it2 != s.end(); it2++) 
	    {
    ...
    {
      for (StringSymbolSet::iterator it = s.begin(); it != s.end(); it++) 
	{
...
Node *Transducer::expand_nodes( Node *node, Transducer *a, StringSymbolSet &s )
{
...
Transducer &Transducer::expand(StringSymbolSet &s)
