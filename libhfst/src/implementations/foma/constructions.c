...
/* int sort_cmp(const struct fsm_state *a, const struct fsm_state *b) { */
...
/*     net1->states = fsm_state_close(); */
/*     net1->statecount = bi_avl_state; */
/*     net1->is_minimized = UNK; */
/*     xxfree(point_a); */
/*     xxfree(point_b); */
/*     xxfree(array); */
/*     bi_avl_free(); */
...
void free_mergesigma(struct mergesigma *msigma) {
    struct mergesigma *prev_msigma;
    for (prev_msigma = NULL; msigma != NULL; ) {
        prev_msigma = msigma;
        msigma = msigma->next;
        xxfree(prev_msigma);
    }
...
void fsm_merge_sigma(struct fsm *net1, struct fsm *net2) {
  ...
  xxfree(mapping_1);
  xxfree(mapping_2);
  free_mergesigma(start_mergesigma);
}
...
/* struct fsm *fsm_minus(struct fsm *net1, struct fsm *net2) { */
/*   return(fsm_intersect(net1,fsm_complement(net2))); */
/* } */
...
struct fsm *fsm_priority_union_lower(struct fsm *net1, struct fsm *net2) {
    /* A .p. B = A | ~[A.l] .o. B */
    return(fsm_union(fsm_copy(net1),fsm_compose(fsm_complement(fsm_lower(fsm_copy(net1))),fsm_copy(net2))));
}
