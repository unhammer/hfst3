...
struct fsm_state_l *state_l_create() {
  struct fsm_state_l *fsm_state_l;
  fsm_state_l = xxmalloc(sizeof(fsm_state_l));
  fsm_state_l->state_no = -1;
  fsm_state_l->in = -1;
  fsm_state_l->out = -1;
  fsm_state_l->target = -1;
  fsm_state_l->final_state = 0;
  fsm_state_l->start_state = 1;
  return fsm_state_l;
}
...
struct fsm_state_l *fsm_state_l_add_arc(struct fsm_state_l *fsm_state_l, int state_no, int in, int out, int target, int final_state, int start_state) {
  if (fsm_state_l == NULL) {
    fsm_state_l = xxmalloc(sizeof(struct fsm_state_l));
  } else {
    for( ; fsm_state_l->next_fsm_state != NULL; fsm_state_l = fsm_state_l->next_fsm_state) {
      /*       printf("ETAOIN SHRDLU!!!\n"); */
    }
    fsm_state_l->next_fsm_state = xxmalloc(sizeof(struct fsm_state_l));
    fsm_state_l = fsm_state_l->next_fsm_state;
  }
  fsm_state_l->next_fsm_state = NULL;
  fsm_state_l->state_no = state_no;
  fsm_state_l->in = in;
  fsm_state_l->out = out;
  fsm_state_l->target = target;
  fsm_state_l->final_state = final_state;
  fsm_state_l->start_state = start_state;
  return(fsm_state_l);
}
...
int find_arccount_l(struct fsm_state_l *fsml) {
  int i;
  for (i=0; fsml != NULL ; fsml = fsml->next_fsm_state)
    i++;
  return i;
}

int fsm_state_l_add_simple_final(struct fsm_state_l *fsml, int number) {

  struct fsm_state_l *temp_fsml, *new_fsml;
  for (;; fsml = fsml->next_fsm_state) {
    if ((fsml == NULL) || (fsml->state_no > number) || (fsml->next_fsm_state == NULL) || (fsml == NULL) || (fsml->state_no == -1)) {
      if (fsml != NULL) {
	/* Interject final non-arc state */
	temp_fsml = fsml->next_fsm_state;
	new_fsml = xxmalloc(sizeof(struct fsm_state_l));
	fsml->next_fsm_state = new_fsml;
      } else {
	new_fsml = xxmalloc(sizeof(struct fsm_state_l));
	temp_fsml = NULL;
      }
      new_fsml->state_no = number;
      new_fsml->in = -1;
      new_fsml->out = -1;
      new_fsml->target = -1;
      new_fsml->final_state = 1;
      if (number == 0) {
	new_fsml->start_state = 1;
      } else {
	new_fsml->start_state = 0;
      }
      new_fsml->next_fsm_state = temp_fsml;
      break;
    }
  }
  return(1);
...
int fsm_state_l_mark_final(struct fsm_state_l *fsml, int number) {
  struct fsm_state_l *fsml_start;
  int found ;
  found = 0;
  fsml_start = fsml;
  for (; fsml != NULL ; fsml = fsml->next_fsm_state) {
    if (fsml->state_no == number) {
      found = 1;
      fsml->final_state = 1;
    }
  }
  if (found == 0) {
    fsm_state_l_add_simple_final(fsml_start, number);
  }
  return(1);
}
...
void fsm_state_l_to_fsm_state(struct fsm *net, struct fsm_state_l *fsml, int arccount) {
    ...
    struct fsm_state *fsm;
    struct fsm_state_l *fsmprev;
    struct sigma *sig;
    int i, skip, maxsigma;
    maxsigma = 0;
    for (sig = net->sigma; sig != NULL ; sig=sig->next) {
        maxsigma = sig->number > maxsigma ? sig->number : maxsigma;
    }
    maxsigma++;
    inoutarr = xxmalloc(sizeof(struct inoutarr)*(maxsigma*maxsigma));
    ...
    fsm = xxmalloc((sizeof(struct fsm_state))*(arccount+1));
    for (i=0;;) {
        skip = 0;
        (fsm+i)->state_no = fsml->state_no;
        (fsm+i)->in = fsml->in;
        (fsm+i)->out = fsml->out;
        (fsm+i)->target = fsml->target;
        (fsm+i)->final_state = fsml->final_state;
        (fsm+i)->start_state = fsml->start_state;

        if (fsml->in != -1) {
            if ((inoutarr+fsml->in*maxsigma+fsml->out)->state == fsml->state_no) {
                if ((inoutarr+fsml->in*maxsigma+fsml->out)->target == fsml->target) {
                    /* Nondeterministic, but we've already seen this exact arc and skip it */
                    skip = 1;
                } else {
                    net->is_deterministic = NO;
                }
            }
            if (fsml->in == EPSILON && fsml->out == EPSILON) {
                net->is_deterministic = NO;
                net->is_epsilon_free = NO;
            }
            (inoutarr+fsml->in*maxsigma+fsml->out)->state = fsml->state_no;
            (inoutarr+fsml->in*maxsigma+fsml->out)->target = fsml->target;
        }

        fsmprev = fsml;
        fsml = fsml->next_fsm_state;
        if ((fsmprev->next_fsm_state == NULL)) {
            /* if ((fsml->state_no == -1)) { */
            xxfree(fsmprev);
            /*      xxfree(fsml); */
            break;
    ...
    (fsm+i)->final_state = -1;
    (fsm+i)->start_state = -1;
    /* qsort(fsm, find_arccount(fsm), sizeof(struct fsm_state), dfa_sort_cmp); */
    net->states = fsm;
    xxfree(inoutarr);
    return;
}
