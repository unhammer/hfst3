	  ...
	  for (iterator it = begin(); it != end(); it++)
	    {
	      ...
	      std::vector<typename HfstTransitionSet::iterator> old_transitions;
		   ...
		   tr_it != it->second.end(); tr_it++)
		{
		  ...
		  /*
		  std::cerr << "Processing transition:\n";
		  std::cerr <<  it->first << "\t" 
			    <<  tr_it->get_target_state() << "\t"
			    <<	data.input_symbol.c_str() << "\t"
			    <<	data.output_symbol.c_str() << "\t"
		  ...
		  */
		  ...
		  if (data.input_symbol.compare(old_symbol) == 0) {
		  ...
		  if (data.output_symbol.compare(old_symbol) == 0) {
		  ...
		  if (substitution_made) {
		    ...
		    /*
		    std::cerr << "Inserting new transition:\n";
		    std::cerr << new_transition.get_target_state() << "\t"
			      << new_transition.get_transition_data().input_symbol.c_str() << "\t"
			      << new_transition.get_transition_data().output_symbol.c_str() << "\t"
			      << new_transition.get_transition_data().weight << "\n";
		    */
	      ...
	      for (typename std::vector<typename HfstTransitionSet::iterator>::iterator IT =
		     old_transitions.begin(); IT != old_transitions.end(); IT++) {
		it->second.erase(*IT);
	      }
	      for (typename HfstTransitionSet::iterator IT = new_transitions.begin();
